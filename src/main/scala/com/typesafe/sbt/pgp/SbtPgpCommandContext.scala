...
/** This class is thrown if grabbing the PGP password fails.
 * Note: We should try to redesign the application rather than using a
 * hacky throw-catch solution in the future.
 */
case class AbortPgpSigningException(msg: String) extends Exception(msg)
...
case class SbtPgpCommandContext(
  ...
  def inputPassphrase = readHidden("Please enter PGP passphrase (or ENTER to abort): ") match {
    ...
    case _ => throw new AbortPgpSigningException("Empty passphrase. aborting...")
  }
